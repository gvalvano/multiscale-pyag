...
import os
from math import pi
from idas.utils import print_yellow_text, get_available_gpus
...
class DatasetInterface(object):
    ...
    def _data_augmentation_ops(self, x_train, y_train_scrib, y_train_mask):
        ...
        y_train_scrib = tf.reshape(y_train_scrib, shape=[-1, self.input_size[0], self.input_size[1], self.n_classes])
        y_train_mask = tf.reshape(y_train_mask, shape=[-1, self.input_size[0], self.input_size[1], self.n_classes])
        ...
        y_train_scrib = tf.contrib.image.rotate(y_train_scrib, angles[0], interpolation='NEAREST')
        y_train_mask = tf.contrib.image.rotate(y_train_mask, angles[0], interpolation='NEAREST')
        ...
        y_train_scrib = tf.contrib.image.translate(y_train_scrib, translations, interpolation='NEAREST')
        y_train_mask = tf.contrib.image.translate(y_train_mask, translations, interpolation='NEAREST')
        ...
        y_train_scrib = tf.cast(y_train_scrib, tf.float32)
        y_train_mask = tf.cast(y_train_mask, tf.float32)
        ...
        return x_train, y_train_scrib, y_train_mask
    ...
    def data_parser(self, filename, standardize=False):
        ...
        fname_scrib = filename.decode('utf-8') + '_scribble.npy'

        batch = np.load(fname).astype(np.float32)
        batch_scrib = np.load(fname_scrib).astype(np.float32)
        ...
        assert not np.any(np.isnan(batch_scrib))

        return batch, batch_scrib, batch_mask
        ...
        with tf.name_scope('{0}_data'.format(self.dataset_name)):
            ...
            train_data = train_data.map(
                lambda filename: tf.py_func(  # Parse the record into tensors
                    ...
                    Tout=[tf.float32, tf.float32, tf.float32]), num_parallel_calls=num_threads)
            ...
            if augment:
                train_data = train_data.map(lambda x, y, z: self._data_augmentation_ops(x, y, z), 
            ...
            test_data = test_data.map(lambda x, y, z: (x[0], y[0], z[0]), num_parallel_calls=num_threads)
